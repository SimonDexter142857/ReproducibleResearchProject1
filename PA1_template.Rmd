...
plot(s4$StepsAvg ~ s4$interval, type = 'n', 
     main='Steps (Avg.) per Interval Mon-Fri vs. Sat-Sun',
     xlab = 'Interval',
     ylab = 'Steps (avg.) per interval',
     sub = 'Missing values imputed');

lines(s4.1$StepsAvg ~ s4.1$interval, 
      type = 'l', lty = 1, lwd = 2, col = 'darkslategray4');

lines(s4.2$StepsAvg ~ s4.2$interval, 
      type = 'l', lty = 1, lwd = 2, col = 'darkseagreen3');

legend('topright', legend = c('Mon-Fri', 'Sat-Sun', 'Delta'), 
                                bty='n',
                                lwd = c(2, 2, 0.5), lty =  c(1, 1, 3) ,
...
s5$Delta = s5$Delta / 5 + 100;

lines(s5$Delta ~ s5$interval, 
      type = 'l', lty = 3, lwd = 0.5, col = 'red');
```
